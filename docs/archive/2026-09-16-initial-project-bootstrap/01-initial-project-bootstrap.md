...
新規Gitリポジトリで`create-project-work-flow`を実行すると、利用者が選択したnpmまたはpnpmに合わせて、開発ワークフローの設定、テンプレート、検証入口、Node.js基盤を安全に生成できる。既存コードを含むリポジトリでは停止し、利用者の変更を上書きしない。
...
- [x] 固定tarballから所有ファイルだけを更新でき、利用者の変更があれば停止する
...
- `npm run test:setup`: 15件成功（npm/pnpm、dry-run、衝突、再実行、tarball更新、install成功・失敗時ロールバック、symlink境界、生成後verify/workflow:prepareを含む）。
- `npm run typecheck`: 成功。
- `npm test`: 230件成功。
- `npm run verify`: 5項目成功。
- tarballを`npm exec --package`で実行するfixtureでも初期化・更新・`npm run verify`・`workflow:prepare`が成功。
