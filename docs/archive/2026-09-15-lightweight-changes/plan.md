...
- [x] 次の任意入力を型とスキーマに追加し、宣言時だけ必要チケットを省略する。対象テストと型検証を実行する。
...
- [x] 空の分類理由を渡すCLIテストを追加し、失敗を見てから空白理由を拒否する。
- [x] 分類確認なし／否定のCLIテストを追加し、失敗を見てから完了時にtrueを要求する。
- [x] 高リスク・変更特性との矛盾を渡すCLIテストを追加し、失敗を見てから拒否する。
- [x] JSON型不正、検証やレビュー不足、通常扱いへの復帰をCLIで検証する。既存条件が拒否していれば本体は変更しない。
- [x] 軽微変更についてHEAD・未コミット変更・検証中の記録変更／削除・検証失敗をCLIで検証する。既存の保護を維持する。
- [x] 現行仕様・README・エージェント指示を同期する。理由・分類確認の記載例、通常扱いへの復帰、意味変更の対象外例を含める。
- [x] 対象ファイルのテストと型検証を繰り返し、最後にnpm run verifyを実行する。code-reviewのStandards／Specを独立したエージェントで実施する。
- [x] レビュー後の根拠をチケットへ記録し、完了した文書を整理する。人間承認の条件は維持し、一時マニフェストで別途管理する。

## 実装後の承認ゲート

検証済み差分を現在のブランチへコミットし、SHAを人間承認へ提示する。
承認前にcompleteやfinalizeにしない。承認記録は一時マニフェストだけに記載する。
承認後の文書更新によってSHAが変わる循環を避けるため、利用者合意によりチケットの完了とは分離する。
