...
function parseOptions(argv) {
    ...
    if (command === 'init' && from)
        fail('--fromはupdateで指定してください');
...
function sourceTemplateRoot(source, requireTarball = false) {
    if (!source)
        return { root: bundledTemplateRoot(), version: FRAMEWORK_VERSION };
    ...
    let version;
    try {
        const packageJson = JSON.parse(readFileSync(join(extraction, 'package/package.json'), 'utf8'));
...
function manifestFor(files, projectName, packageManager, frameworkVersion = FRAMEWORK_VERSION) {
    return {
        schemaVersion: 1,
